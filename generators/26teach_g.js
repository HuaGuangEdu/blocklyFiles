...
Blockly.Python.closeled= function () {
    ...
    var mode = this.getFieldValue('DIR');
    if(mode=='closeled')
    {var code = led+'.closeled()\n';}
    else if(mode=='openled')
    {var code = led+'.openled()\n';}
    return code;
};
...
Blockly.Python.motor_single= function () {
    ...
    var mode = this.getFieldValue('DIR');
    if(mode=='go')
    {var code = motor +'.motor_single(0,'+num+')\n';}
    else if(mode=='back')
    {var code = motor +'.motor_single(1,'+num+')\n';}
    return code;
};
...
Blockly.Python.driverless_car_raspberrypi_car_xunxian= function () {
    Blockly.Python.definitions_['control_gpio'] = 'from control import gpio';
    Blockly.Python.definitions_['control'] = 'from control import yuyin';
    Blockly.Python.definitions_[' '] = 'import time';
    Blockly.Python.definitions_['control_integration'] = 'from control import integration';
     var b = Blockly.Python.valueToCode(this, 'car_m', Blockly.Python.ORDER_ATOMIC);
     var code =
                'integration.case_line_patrol('+ b +')'
     return code;
};
