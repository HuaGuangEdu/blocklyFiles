...
Blockly.Blocks['logic_is_in'] = {
  init: function() {
    this.setColour(210);
    this.appendValueInput('A_in',String)
         .setCheck("String");
    this.appendValueInput('B_in',String)
        .setCheck("String")
        .appendField("在");
...
Blockly.Constants.Logic.LOGIC_COMPARE_ONCHANGE_MIXIN = {
  ...
  onchange: function(e) {
    ...
    //实例化连接检查器
    var checker = Blockly.ConnectionChecker;
    var checkers = new checker();
    if (blockA && blockB && !checkers.doTypeChecks(blockA.outputConnection,blockB.outputConnection)) {
  ...
  onchange: function(e) {
    ...
    if ((blockA || blockB) && parentConnection) {
      for (var i = 0; i < 2; i++) {
        ...
        block.IConnectionChecker.canConnect();
        //实例化连接检查器
        var checker = Blockly.ConnectionChecker;
        var checkers = new checker();
        if (block && !checkers.doTypeChecks(block.outputConnection,parentConnection)) {
